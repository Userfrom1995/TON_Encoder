...
public:
    ...
    {   
        std::cout<<"entered compress"<<"\n";
        vm::CellBuilder result_cb;
        size_t pos = 0;

        // First pass: build dictionary of repeated patterns
        std::cout<<pos<<" "<<min_pattern_size<<" "<<data.size()<<"\n";
        while (pos + min_pattern_size <= data.size())
        { 

            // std::cout<<max_pattern_size<<"min :"<<min_pattern_size<<"\n";
            ...
            pos++;
            // std::cout<<"ended for loop"<<"\n";
        }

        std::cout<<"ended while loop"<<"\n";
            ...
            for (size_t len = max_pattern_size; len >= min_pattern_size; len--)
            {
                ...
                for (int i = 0; i < next_pattern_id; i++)
                {
                    ...
                    if (!dict_entry.is_null())
                    {
                        ...
                        vm::CellSlice cs(vm::NoVm(), dict_entry.write().get_base_cell());
        ...
        while (!cs.empty())
        {
            ...
            if (is_pattern)
            {
                ...
                if (!pattern_entry.is_null())
                {
                    ...
                    vm::CellSlice pattern_cs(vm::NoVm(), pattern_entry.write().get_base_cell());
